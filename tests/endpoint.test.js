...
test('Device-verified passkey session opens administration but cannot bypass roles or revocation',async()=>{
 aal='aal1';passkeyGrant=false;
 assert.equal((await request('publish',env.ADMIN_ORIGIN,{verifiedPasskey:true,strongAuth:true})).status,403);
 passkeyGrant=true;
 const s=await (await request('session')).json();assert.equal(s.strongAuth,true);assert.equal(s.aal,'aal1');
 role='admin';assert.equal((await request('publish')).status,200);
 role='editor';assert.equal((await request('publish')).status,403);
 role='viewer';assert.equal((await request('users')).status,403);
